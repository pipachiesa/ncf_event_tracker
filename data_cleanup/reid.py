...
def compute_descriptors(video_path, tracks, n_crops=N_CROPS, fps=DEFAULT_FPS):
    ...
    try:
        ...
        frame_no = 1
        while frame_no <= last_needed:
            ok, img = cap.read()
            if not ok:
                break
            for tid, bbox in needed.get(frame_no, ()):
                crop = _torso_crop(img, bbox)
                if crop is None:
                    continue
                h = _crop_histogram(crop)
                if h is not None:
                    scored[tid].append((_crop_sharpness(crop), h, frame_no))
            frame_no += 1
    finally:
        cap.release()
...
def run_reid(tracking_csv, video, output, tmax=DEFAULT_TMAX_S,
             dmax=DEFAULT_DMAX_CM, smin=DEFAULT_SMIN, fps=DEFAULT_FPS,
    ...
    compute_descriptors(video, tracks, fps=fps)
